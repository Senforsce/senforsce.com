...
export default function updateAll(letterA, letterS, circle, a_l_center, a_r_center) {
	...
	var radiusScale = d3.scaleLinear().domain(domain).range([radius - minWidth, radius - maxWidth]);
	var colorScale = d3.scaleLinear().domain(domain).range([minColor, maxColor]);



	var gauge = circle.selector.
	    select('#gauge').attr("transform", "translate(" + 702 + "," + 383 + ")");
	...
	letterA.left.attr("transform", "translate(20, -10) rotate(-120 " + a_l_center + ")")
	    .attr("stroke-width", 0);
	    letterA.right.attr("transform", "translate(0, 0) rotate(0 " + a_r_center + ")")
	    .attr("stroke-width", 0);
